...
import { useDeleteCareers } from "hooks/careers/useDeleteCareers";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deletecareers = useDeleteCareers({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useCareers();
  const DeleteCareers = () => {
    setLoading(true);
    deletecareers.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteCareers}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
