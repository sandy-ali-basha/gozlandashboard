...
import { useDeleteProduct_type } from "hooks/product_type/useDeleteProduct_type";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deleteproduct_type = useDeleteProduct_type({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useProduct_type();
  const DeleteProduct_type = () => {
    setLoading(true);
    deleteproduct_type.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteProduct_type}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
